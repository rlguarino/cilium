...
func (n *EndpointSelector) UnmarshalJSON(b []byte) error {
	...
	if n.MatchLabels != nil {
		...
		for k, v := range n.MatchLabels {
			ml[labels.ParseExtendedKey(k)] = v
		}
		n.MatchLabels = ml
	}
	if n.MatchExpressions != nil {
		...
		for i, v := range n.MatchExpressions {
			v.Key = labels.ParseExtendedKey(v.Key)
			newMatchExpr[i] = v
		}
...
func (n EndpointSelector) MarshalJSON() ([]byte, error) {
	...
	if n.MatchLabels != nil {
		...
		for k, v := range n.MatchLabels {
			l := labels.ParseLabel(k)
			newLabels[l.Source+":"+l.Key] = v
		}
	...
	if n.MatchExpressions != nil {
		...
		for i, v := range n.MatchExpressions {
			l := labels.ParseLabel(v.Key)
			v.Key = l.Source + ":" + l.Key
			newMatchExpr[i] = v
		}
